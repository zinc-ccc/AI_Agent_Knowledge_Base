...
- **最佳实践【无需手写，智能命名，AI 全程代劳】**：立项的第一天，请在你的**项目根目录**建一个 Markdown 格式的项目操作手册。**你完全不需要手写文档，也不需要主动催促 AI 更新**。
  - **初次接管与自动命名法则**：当 Agent 协助任何一个新项目时，**必须第一时间**主动检查根目录是否存在操作手册。若无，必须不经询问立刻创建。为了避免部门共享时的冲突，AI 必须严格执行带名字的格式化命名：**`[你的姓名全拼或英文名]_[项目功能]_项目操作手册.md`**（例如：`Zinc_周报自动化分发_项目操作手册.md`）。所有操作必须在这个文件里记录！
