...
### 4. 规范先行：项目文档与智能维护
- **痛点**：大家都在凭兴趣摸索各种 AI 提效工具。如果各搞各的，不仅代码难以看懂，工具链难以融合，且每次项目迭代后文档往往跟不上，最终自动化逻辑成为“黑盒”。
- **最佳实践【无需手写，全程让 AI 代劳与追踪】**：立项的第一天，请在你的**项目根目录**建一个 Markdown 格式的《项目操作手册.md》。**你完全不需要手写文档，也不需要主动催促 AI 更新**。在接手和开发项目的过程中：
  - **初次接管的底线原则**：当 Agent 协助任何一个新项目时，**必须第一时间**主动检查根目录是否存在《项目操作手册》。若无，必须不经询问立刻创建。
  - **智能捕获与全局同步更新**：文档绝不能是一次性的死物。在整个开发迭代周期中，Agent 有不可推卸的责任进行**智能侦听与维护**。无论是修复了一个 bug、改变了部署途径（比如从飞书换到了网页发布），还是踩坑后调整了策略，Agent 必须**自主感知**这些业务目的和功能变动，并将最新逻辑和复用经验实时同步沉淀至相关联的规则文档及操作手册中。
  - **贡献溯源**：要在文档显眼位置署名。未来这篇由 AI 自主打理、记录了详细技术路线和填坑经验的规范，将是复用至部门大系统的核心名片。
