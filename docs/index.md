...
### 4. 规范先行：项目文档与智能维护
- **痛点**：大家都在凭兴趣摸索各种 AI 提效工具。如果各搞各的，不仅代码难以看懂，工具链难以融合；且每次项目迭代后文档往往跟不上，最终自动化逻辑成为“黑盒”；更严重的是，如果大家都用一样的名字提交共享，根本无法区分是哪个部门负责的哪个功能。
- **最佳实践【无需手写，智能命名，AI 全程代劳】**：立项的第一天，请在你的**项目根目录**建一个 Markdown 格式的项目操作手册。**你完全不需要手写文档，也不需要主动催促 AI 更新**。
  - **初次接管与自动命名法则**：当 Agent 协助任何一个新项目时，**必须第一时间**主动检查根目录是否存在操作手册。若无，必须不经询问立刻创建。为了避免部门共享时的冲突，AI 必须严格执行带名字的格式化命名：**`[飞书全英文名]_[项目功能]_项目操作手册.md`**（例如：`Zinc.Zheng_周报自动化分发_项目操作手册.md`）。所有操作必须在这个文件里记录！
  - **智能捕获与全局同步更新**：文档绝不能是一次性的死物。在整个开发迭代周期中，Agent 有不可推卸的责任进行**智能侦听与维护**。无论是修复了一个 bug、改变了策略，Agent 必须**自主感知**这些业务变动，实时沉淀进手册里。遇到有团队复用价值的新经验，需准备向部门核心沉淀库发起无缝回传。
  - **贡献溯源**：要在文档显眼位置署名。这篇由 AI 自动命名并自主打理、记录了详细技术路线和填坑经验的专业规范，将是未来复用至大系统的核心名片。
