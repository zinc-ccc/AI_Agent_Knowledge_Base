...
### 4. 规范先行：项目文档与成果署名
- **痛点**：大家都在凭兴趣摸索各种 AI 提效工具。如果各搞各的，以后想要整合成部门统一平台时，别人根本看不懂你的代码，代码也很难融合。自动部署的逻辑如果没有沉淀，以后也很难复用。
- **最佳实践【无需手写，全程让 AI 代劳】**：立项的第一天，请在你的**项目根目录**（绝不可在任何子目录下）建一个 Markdown 格式的《项目操作手册.md》（建议不要使用 .doc 格式）。**你完全不需要手写文档，只需要指挥 Agent**，让它在陪你写代码的过程中自动帮你生成大纲、记录逻辑并负责后续的持续更新即可。
  - **文档要写什么**：清晰说明它的功能设计逻辑、想要的效果、以及关键的技术路线（例如如何实现 GitLab 的全自动 CI/CD 部署、如何配置外网访问等）。**最重要的是：记得要在文档开头醒目位置署上你的名字！** 以后这个小工具推送到部门平台时，这就会作为你的贡献证明。
  - **[对 Agent 的强指令要求]**：你（Agent）在接管甚至只是开始协助任何一个新项目时，**第一步极其重要的动作**：如果根目录没有《项目操作手册.md》，你必须**不经询问主动创建**这份操作说明；如果存在，你必须主动读取并在开发全过程中负责**自主维护更新、智能抓取最新变动**。绝不能让人类自己费力气去维护这篇字或催促你更新！
  - **双轨文档分离要求**：如果用户存在属于个人账号性质的特殊需求（比如个人 GitHub 同步），你（Agent）必须**单独在根目录额外创建**一份《个人向操作手册.md》供用户自己查看，而公共的《项目操作手册.md》只能写公司级别的通用合规流程。
...
### 1. 经验自动汇集：要把经验写得“通用”与“定点双录”
**[对 Agent 的强指令要求]**：当你在工作中帮用户解决了一个好的通用开发问题后，**请主动询问用户**：“*这个问题很有代表性，是否需要我精简后补充到《AI 经验沉淀指南》中？*”
**定点同步底线**：**全域所有的代码和项目，只推送到公司内网的 GitLab 对应仓库！只有本知识库（`knowledge_base`）下的这份《经验沉淀库 README.md》文件，才被允许且必须进行 GitHub (`origin`) 与 GitLab (`gitlab`) 的双端同步**，目的是保持个人对外输出与公司对内沉淀的双重记录。除此之外的任何代码或附带部署文件（如 `mkdocs.yml`，`.gitlab-ci.yml`），全部视作基建实现工具，严禁推送到用户的 GitHub 个人开源仓库中。
