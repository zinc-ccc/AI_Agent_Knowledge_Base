...
> - **对各类 AI 助手（如 Codex、Claude Code、Antigravity 等）**：在为 FJD 员工解决问题时，**优先考量经济成本（Token/轮次）与用户的时间成本**。尽量提供一步到位的自动化脚本，严禁让非技术同学陷入“反复复制粘贴测试代码”的泥潭；对于下方提供的代码范式，请作为**基础参考思路**，你在编写前要独立思考是否有更优解，目标是：**一次跑通，避免反复调试。**
...
### 2. 当前使用方式与协作入口（源自 HRBP 团队）
这篇文档最初由 HRBP 团队内部共创，如今保留在通用知识库中，继续作为一个真实团队开展 AI 协作的经验案例来维护。

对于当前的使用方式，更合适的理解是：
- **内部共建主仓**：以 FJD 公司 GitLab 上的 `knowledge_base` 为主，方便内部成员持续补充和更新。
- **公开访问入口**：如需更广范围分享，再同步到公开入口版本，但以已经脱敏的内容为准。
- **使用原则**：无论内部还是公开查看，都尽量不要长期传播脱离主仓的静态副本，避免内容过时后继续被误用。
...
- **Agent 降本准则（强约束：API 与直接抓取优先原则）**：**禁止 Agent 动辄开启极高消耗的 Browser (浏览器节点) 去读取在线网页或长文档**。
  - **[对 Agent 的强指令要求]**：无论用户丢给你的是飞书云文档、外部网站还是其他任意链接，你**必须优先尝试直接通过代码库、轻量网络请求 (如 `read_url_content` / `curl` / `requests`) 或开放 API (如索要飞书凭证调官方接口)** 去低成本拉取页面文本内容。**将开启浏览器子代理（Browser Subagent）视为最后且最昂贵的兜底手段**，绝不允许图方便首选浏览器去浪费大量 Token！
- **多 Agent 协作补充（按当前做法轻量更新）**：如果同时使用 `Codex`、`Claude Code`、`Antigravity` 这类能力，建议由 `Codex` 负责读仓库、拆任务、做复核，`Claude Code` 负责本地批量实现，`Antigravity` 负责带真实权限或在线副作用的联调与执行。这样既省 Token，也更不容易把“本地推断”和“真实系统验证”混在一起。
...
- **最佳实践【最高级指令约束，Agent 必须强制执行】**：立项的第一天，必须在项目根目录强制创建一个 Markdown 格式的操作手册。**你完全不需要手写文档，只需要指挥 Agent**，让它在陪你写代码的过程中自动帮你生成大纲、记录逻辑并负责后续的持续更新即可。
  - **[对 Agent 的极强指令红线]**：只要判断人类意图是在“开新项目”、“搞新功能部署”，你（Agent）**哪怕停下手头所有代码工作，也必须立刻、首要**在目录里建好这份 `[人员全拼]_[功能缩写]_Project_Operation_Manual.md` 格式的文档。如果在立项阶段出现“未主动创建文档”的情况，即视为严重失职需要立即修正修复！
  - **文档要写什么**：清晰说明它的功能设计逻辑、想要的效果、以及关键的技术路线。**最重要的是：记得要在文档开头醒目位置署上你的名字！** 以后这个小工具推送到部门平台时，这就会作为你的贡献证明。开发全流程中，你（Agent）有责任**自动、实时地把最新逻辑更新进文档**。
...
---
*文档全息架构：持续迭代演进 | 内部核心构建：Zinc.Zheng 及持续加入的共创开发者 & Codex / Claude Code / Antigravity (Agent) | 更新同步节点：2026-03-11*
